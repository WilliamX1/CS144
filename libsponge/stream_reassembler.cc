...
void StreamReassembler::merge_substrings() {
    ...
    if (iter_next != _substrings.end()) ++iter_next;
    ...
    while (iter_next != _substrings.end()) {
        ...
        if (next_end <= prev_end) iter_next = _substrings.erase(iter_next);
...
void StreamReassembler::write_substrings() {
    while (!_substrings.empty()) {
        ...
        cout << "write_substrings:  idx_begin: " << idx_begin << " idx_end: " << idx_end << "\n";
        if (_nextbyte >= idx_end) _substrings.erase(iter);
        else if (_nextbyte < idx_begin) break;
        else { 
            // just regard the piece as a incoming part and reuse the function to handle it
            _substrings.erase(iter);
            push_substring(iter->data, iter->index, iter->eof);
        };
    };
}
...
void StreamReassembler::push_substring(const string &data, const size_t index, const bool eof) {
    ...
    if (_nextbyte >= index_end) {}
        ...
        // insert and combine the part into the _substrings
        auto iter = _substrings.begin();
        while (iter != _substrings.end()) {
            if (iter->index + iter->data.size() >= index_begin) break;
            else continue;
        };
    ...
    merge_substrings();
    discard_substrings();
    cout << "_eof: " << _eof << " _next_bytes: " << _nextbyte << "_end_index: " << _end_index << "\n";
    if (empty())
        _output.end_input();
    // cout << "_output bytestream size:  " << _output.buffer_size() << "\n";
    return;
}
...
bool StreamReassembler::empty() const {
    return _eof && _nextbyte >= _end_index;
}
